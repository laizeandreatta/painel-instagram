...
import { useLeads, RECURSO_REPUTACAO } from "@/lib/useLeads";
...
// CRM Reputação Digital: leads do Método Reputação que Vende, desde a
// aplicação enviada no formulário do site (www.laizeandreatta.com.br,
// via /api/reputacao/aplicacao) até a reunião confirmada na agenda do
// Google. Usa o mesmo funil de vendas do CRM Consultoria (LEAD_STATUS_ORDER).
export default function CrmReputacaoPage() {
  const { leads, loading, criarLead, atualizarStatus, excluirLead, recarregar } =
    useLeads(RECURSO_REPUTACAO);
  ...
  // Confere na hora, na agenda do Google, se alguém marcou a reunião pelo
  // calendário liberado após enviar a aplicação no site (veja
  ...
  async function verificarAgendamentos() {
    ...
    try {
      ...
      if (!resp.ok || !json.ok) {
        ...
        window.alert(
          `${json.leadsAtualizados} lead(s) marcados como "Reunião agendada".`
        );
        recarregar();
      } else {
    ...
    <div className="px-6 py-7 md:px-10">
      <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-editorial text-2xl font-semibold text-ink md:text-3xl">
            CRM Reputação Digital
          </h1>
          <p className="text-sm text-ink/50">
            Leads do Método Reputação que Vende: da aplicação enviada no
            site até a conversa marcada.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-line bg-white px-4 py-10 text-center text-sm text-ink/40">
          Nenhum lead cadastrado ainda. Assim que alguém enviar a aplicação
          no site, o lead vai aparecer aqui automaticamente — ou cadastre
